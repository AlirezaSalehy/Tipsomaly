...
class _Model(nn.Module):
  """Text transformer similar to CLIP, with optional prompt learning."""
  ...
  def __call__(self,
               ...
               *,
               train: bool = False,
               # NEW: optional prompt-learning inputs (all default to None -> no-op)
               learnable_prompts: Optional[jnp.ndarray] = None,  # [P, D]
               learning_method: Optional[str] = None,            # {"concat","sumate","entire_learnable", None}
               deep_parameters: Optional[list] = None            # optional, see NOTE (per-layer deep prompts)
               ):
    """Args:
      text: [N, L] token ids (EOS-padded; EOS id must be 1 as in original code).
      learnable_prompts: prompt tokens to apply; shape [P, D] where D == width.
      learning_method: 'concat' | 'sumate' | 'entire_learnable' | None.
      deep_parameters: placeholder for per-layer prompt tokens (see NOTE).
    """
    del deep_parameters  # NOTE: see "Deep prompts" section below.

    out = {}

    embedding = nn.Embed(num_embeddings=self.vocab_size, features=self.width)
    x = out["embedded"] = embedding(text)  # [N, L, D]
    n, l, d = x.shape

    # -------------------------------
    # PROMPT LEARNING (optional)
    # -------------------------------
    if learnable_prompts is not None and learning_method:
      # Validate dimensionality early.
      p, d_p = learnable_prompts.shape
      if d_p != d:
        raise ValueError(f"learnable_prompts has dim {d_p}, expected {d}.")

      # Broadcast prompts across batch: [N, P, D]
      prompts_b = jnp.broadcast_to(learnable_prompts[None, ...], (n, p, d))

      if learning_method == "concat":
        # Prepend prompts so EOS (last token) stays last.
        x = jnp.concatenate([prompts_b, x], axis=1)  # [N, P+L, D]

      elif learning_method == "sumate":
        # Add prompts element-wise to the first P tokens.
        if p > l:
          raise ValueError(f"sumate needs P <= L, got P={p}, L={l}.")
        x = x.at[:, :p, :].add(prompts_b)

      elif learning_method == "entire_learnable":
        # Replace by prompts, but append an EOS token so 'last' pooling remains valid.
        eos_ids = jnp.ones((n, 1), dtype=text.dtype)         # EOS id is 1 in this codebase
        eos_embed = embedding(eos_ids)                        # [N, 1, D]
        x = jnp.concatenate([prompts_b, eos_embed], axis=1)  # [N, P+1, D]
      else:
        raise ValueError(f"Unknown learning_method: {learning_method}")

      # Update sequence length after prompt ops.
      _, l, _ = x.shape

    # -------------------------------
    # Positional embeddings (unchanged logic, but after prompt ops)
    # -------------------------------
    x = x + self.param(
        "pos_embedding",
        nn.initializers.normal(stddev=1/np.sqrt(d)),
    ...
    # Encoder (kept identical; see NOTE to enable deep prompts)
    x, encoder_out = vit.Encoder(
        depth=self.depth, mlp_dim=self.mlp_dim, num_heads=self.num_heads,
        scan=self.scan, remat_policy=self.remat_policy, dropout=self.dropout)(
            x, deterministic=not train)
    ...
    # Share weights between embeddings and logit transformation (unchanged).
    out["vocab_logits"] = embedding.attend(x)

    # Pooling (unchanged semantics).
    if self.pool_type == "last":
      # With 'concat', EOS is still last. With 'entire_learnable', we explicitly appended EOS.
      x = out["pre_logits"] = x[:, -1, :]
    elif self.pool_type == "first":
      x = out["pre_logits"] = x[:, 0, :]
    elif self.pool_type in ("mean", "gap"):
      ...
      x = out["pre_logits"] = vit.MAPHead(num_heads=self.num_heads, mlp_dim=self.mlp_dim)(x)
    else:
      raise NotImplementedError(f"Cannot do pooling '{self.pool_type}'")
